...
export async function verifyFeishuApp({ appId, appSecret, domain = "feishu", fetchImpl = fetch } = {}) {
  const tokenResponse = await fetchImpl(`${originFor(domain)}/open-apis/auth/v3/tenant_access_token/internal`, {
    method: "POST",
  ...
  if (!tokenBody?.tenant_access_token) {
    throw Object.assign(new Error("飞书应用无法验证。"), { code: "verify-failed" });
  }
  const botResponse = await fetchImpl(`${originFor(domain)}/open-apis/bot/v3/info/`, {
...
export async function registerFeishuApp(options = {}) {
  ...
  if (typeof lark?.registerApp !== "function") {
    throw Object.assign(new Error("当前飞书 SDK 不能扫码开通。请改用手动填写。"), { code: "qr-unavailable" });
  }
